...
//---------- Otros --------//
  ...
  float den = (Am11*Bm2 - Am21*Bm1)*Cm2 + (Am22*Bm1 - Am12*Bm2)*Cm1; //= -1
  float k2term = (Am11*Bm2 - Am21*Bm1); //= -1
  float k1term = (Am22*Bm1 - Am12*Bm2); //= -1
  float term = (Am12*Am21 - Am11*Am22); //= -1
// ******************************************************** //
//----------  Librerias  --------//                      //
// ******************************************************** //
...
void setup() {
  ...
  // Ganancias de Observador
  //L1 = ((Am12*Cm1 - Am11*Cm2)*(sum_b + Am11*Am22) - (prod_b + Am12*Am21 - Am11*Am22)*Cm2)/((Am12*Cm1 - Am11*Cm2)*Cm1 - (Am21*Cm2 - Am22*Cm1)*Cm2);
  //L2 = ((Am22*Cm1 - Am21*Cm2)+(sum_b + Am11 + Am22) + (prod_b - Am11*Am22 + Am12*Am21)*Cm1)/((Am12*Cm1 - Am11*Cm2)*Cm1 - (Am21*Cm2 - Am22*Cm1)*Cm2);

  //Ganancias Obseraor Mathcad
 ...
 L2 = ((Am21*Cm2 - Am22*Cm1)*sum_b + (Am21*Am22 + Am11*Am21)*Cm2 - (Am22*Am22 + Am12*Am21)*Cm1 - Cm1*prod_b)/(Am21*Cm2*Cm2 + (Am11 - Am22)*Cm1*Cm2 - Am12*Cm1*Cm1);
  ...
  // Servo Sistema
  g = ((Am12*Bm2 - Am22*Bm1)*K1 + (Am21*Bm1 - Am11*Bm2)*K2 + (Am11*Am22 - Am12*Am21))/(Am21*Cm2*Bm1 + Am12*Cm1*Bm2 - Am22*Cm1*Bm1 - Am11*Cm2*Bm2); //Expresión a mano
}
...
void control(){
  ...
  U = g*R - K1*X1 - K2*X2;       // Ley de control retro estado medido
...
void observador(){
  // Dinamica observador
  float f1 = Am11*XeR1 + Am12*XeR2 + Bm1*U + L1*(Y-XeR2);     // Dinamica estado 1 observador
  float f2 = Am21*XeR1 + Am22*XeR2 + Bm2*U + L2*(Y-XeR2);     // Dinamica estado 2 observador
